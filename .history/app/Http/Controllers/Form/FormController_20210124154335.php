    ...
    public function draftForm(Request $request, $form)
    {
        if ($request->ajax()) {
            ...
            foreach ($request->except('_token') as $key => $value) {
                ...
                $key_parts_ = array_reverse($key_parts);
                $field = array_shift($key_parts_);
                $unique_key = array_shift($key_parts_);
                $template = implode('_', array_reverse($key_parts_));
                if($key!=='question-type' && $key!=='field_limit' && $key!=='custom_text' && $key!=='resval'){
                    ...
                    $new_key = "{$template}.{$unique_key}.{$field}";
                    $inputs_res = array_add($inputs, $new_key, $key);
                    // $inputs_res[] = array_add($inputs, $new_key, $value);
                    // $inputs_res = $key_parts;
                }
            }
            ...
            foreach ($form->fields as $field) {
                ...
                // $field->options = data_get($inputs_res, "{$field->attribute}.type");
                $field->filled = true;
                $extra2[]=$inputs;
                $field->save();
            }
