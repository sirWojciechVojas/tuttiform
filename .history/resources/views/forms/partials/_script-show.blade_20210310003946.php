    ...
    $(function() {
        ...
        $('a.question-template').on('click', function (event) {
            ...
            .done(function (response) {
                if (response.success) {
                    ...
                    var $questionInput = $('.questions .panel.short-answer:first-of-type input.question-name');
                    if(data.template=='short-answer' && $questionInput.val()!=='Podaj swój adres email'){
                        $questionInput.val('Podaj swój adres email');
                        if('.questions .panel.short-answer:first-of-type input.question-name')
                        $('#create-form').submit();
                    }
        ...
        function prepareFormTemplatesWithFieldData(fields, template_divs) {
            template_divs.each(function (index) {
                ...
                template.find('input.question-resval').attr({
                    name: field_data['attribute'] + '.resval',
                    id: field_data['attribute'] + '.resval'
                });
                ...
                if (field_data.hasOwnProperty('options')) {
                    ...
                    if (template.hasClass('linear-scale')) {
                        ...
                        if(resvals[field_data.id] !== undefined && template.hasClass('checkboxes')){
                ...
                if (field_data['required'] == true || field_data['required'] === null) {
                    ...
                    //template.find('input.question-required').trigger('change');
                    template.find('.switchery:first-of-type').trigger('click');
                } else {
        ...
        function showError(error) {
            ...
            notify('error', message);
                ...
                // Moje Triggery dodane


        if($('.questions .panel.short-answer:first-of-type input.question-name').val()!=='Podaj swój adres email'){
            $('a.question-template[data-id="short-answer"]').trigger('click');
            //$('.questions .panel.short-answer:first-of-type input.question-name').val('Podaj swój adres email');
        }
        //alert($('.questions .panel.short-answer:first-of-type input.question-name').val());
    });
    function mySelect(){
        // Moje selecty dodane
